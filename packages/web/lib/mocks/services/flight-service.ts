...
export class MockFlightService implements IFlightService {
  ...
  private generateBaggageInfo(airlineCode: string, cabin: string): { carry: string; checked: string } {
    ...
    const cabinBaggage: { [key: string]: { carry: string; checked: string } } = {
  ...
  private selectConnectionAirport(origin: any, destination: any): any {
    // Select a realistic connection airport based on geography and major hubs
    const originCode = origin.code;
    const destinationCode = destination.code;
    
    // Define realistic connection hubs for different regions
    const connectionRoutes: { [key: string]: string[] } = {
      // US East Coast connections
      'ATL-JFK': ['ORD', 'DFW'],
      'ATL-LGA': ['ORD', 'DFW'],
      'ATL-BOS': ['ORD', 'JFK'],
      'JFK-MIA': ['ATL', 'ORD'],
    ...
    // Get possible connections for this route
    let possibleConnections = connectionRoutes[routeKey] || connectionRoutes[reverseRouteKey];
    
    // If no specific route defined, use intelligent defaults based on regions
    if (!possibleConnections) {
      const usEastCoast = ['JFK', 'LGA', 'BOS', 'DCA', 'BWI', 'PHL', 'MIA'];
      const usWestCoast = ['LAX', 'SFO', 'SEA', 'PDX', 'LAS', 'PHX'];
      const usCentral = ['ORD', 'DFW', 'DEN', 'ATL', 'MSP', 'DTW'];
      
      const isOriginEast = usEastCoast.includes(originCode);
      const isDestinationEast = usEastCoast.includes(destinationCode);
      const isOriginWest = usWestCoast.includes(originCode);
      const isDestinationWest = usWestCoast.includes(destinationCode);
      
      if (isOriginEast && isDestinationWest) {
        // East to West: use central hubs
        possibleConnections = ['ORD', 'DFW', 'DEN'];
      } else if (isOriginWest && isDestinationEast) {
        // West to East: use central hubs
        possibleConnections = ['ORD', 'DFW', 'DEN'];
      } else if (isOriginEast && isDestinationEast) {
        // East Coast to East Coast: use major east coast hubs
        possibleConnections = ['ATL', 'ORD', 'JFK'];
      } else if (isOriginWest && isDestinationWest) {
        // West Coast to West Coast: use west coast hubs
        possibleConnections = ['LAX', 'SFO', 'DEN'];
      } else {
        // Default to major hubs
        possibleConnections = ['ATL', 'ORD', 'DFW'];
      }
    }
    
    // Filter out origin and destination from possible connections
    possibleConnections = possibleConnections.filter(code => 
      code !== originCode && code !== destinationCode
    );
    
    // Select random connection from realistic options
    if (possibleConnections.length === 0) {
      possibleConnections = ['ATL', 'ORD', 'DFW']; // Fallback to major hubs
    }
    
    const selectedHub = possibleConnections[Math.floor(Math.random() * possibleConnections.length)];
    return getAirportByCode(selectedHub) || origin;
  }
