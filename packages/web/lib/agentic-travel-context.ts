...
export class AgenticTravelContextManager {
  ...
  enhancePromptWithContext(sessionId: string, userMessage: string): string {
    ...
    if (
      userPreferences.destinations?.length ||
      userPreferences.travelDates?.startDate
    ) {
      contextPrompt += '\n\nTravel Context:';
 ...
 */
export function enhanceWithTravelContext(
  sessionId: string,
  userMessage: string
): string {
 ...
 */
export function extractTravelPreferencesFromMessage(
  message: string
): Partial<UserTravelPreferences> {
  ...
  const travelerMatches = message.match(
    /(\d+)\s*(?:people|person|adult|traveler)/gi
  );
  if (travelerMatches) {
