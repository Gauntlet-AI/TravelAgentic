 ...
 */
export function enhanceTravelPrompt(userMessage: string, context?: TravelContext): string {
  ...
  if (context.startDate && context.endDate) {
    contextParts.push(`Travel dates: ${context.startDate} to ${context.endDate}`)
  }
...
export const travelPromptTemplates = {
  destinationRecommendation: "I'm looking for travel destination recommendations. Here are my preferences:",
  budgetPlanning: "Help me plan a budget for my trip. I want to understand costs for:",
  ...
  transportationHelp: "What are the best transportation options for getting around",
}
...
export function generateTravelSuggestions(input: string): string[] {
  ...
  if (lowerInput.includes('budget') || lowerInput.includes('cost') || lowerInput.includes('expensive')) {
...
export function extractTravelEntities(message: string): Partial<TravelContext> {
  ...
  const budgetPatterns = /\$?\d+(?:,\d{3})*(?:\.\d{2})?|\b(?:budget|cheap|expensive|luxury)\b/gi
  const travelerPatterns = /\b(\d+)\s*(?:people|person|traveler|adult|couple)\b/gi
  const timePatterns = /\b(?:spring|summer|fall|autumn|winter|\d{1,2}\/\d{1,2}\/\d{4}|\d{4}-\d{2}-\d{2})\b/gi
