...
export class TravelContextManager {
   ...
   */
  initializeContext(sessionId: string, flowType: 'structured' | 'conversational' = 'conversational'): TravelContext {
   ...
   */
  updateContext(sessionId: string, updates: Partial<TravelContext>): TravelContext | null {
   ...
   */
  addToolCall(sessionId: string, tool: string, parameters: any, result: any): void {
    const context = this.contexts.get(sessionId)
    if (!context) return
   ...
   */
  addToCart(sessionId: string, type: 'flights' | 'hotels' | 'activities', items: any[]): void {
    const context = this.contexts.get(sessionId)
    if (!context) return
    
    const currentCart = context.shoppingCart || { flights: [], hotels: [], activities: [], totalPrice: 0, cartVersion: 1 }
   ...
   */
  updateWorkflowState(sessionId: string, updates: Partial<TravelContext['workflowState']>): void {
    const context = this.contexts.get(sessionId)
    if (!context) return
